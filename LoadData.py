...
class LoadData(LoadDataInterface):
    ...
    def load_images(self, index_ranges, height, width, bgr=False):
        ...
        x = array(list(map(lambda x: asarray(load_img(self.img_folder_path + x[0], target_size=(height, width), color_mode='rgb')), meta_data))) # load images for file names in meta_data
        if (bgr):
            x = list(map(lambda x: cvtColor(x, COLOR_RGB2BGR), x))
        y = array(list(map(lambda x: x[1], meta_data))) # unpack ground truths from meta_data
